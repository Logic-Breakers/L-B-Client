    ...
    return (
        <div>
            ...
            <div className="flex flex-col justify-start items-start fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] max-w-[1032px] w-[90%] h-[90%] bg-[var(--modal-inner-bg)] rounded-[12px] z-[1000] px-[24px]">
                <button type="button" onClick={closeGlobalModal} className={`text-[16px] px-[8px] py-[2px] rounded-[20px] hover:shadow-inner hover:bg-[#F7F7F7] mt-[20px] ml-[-8px]`}>
                    ✕
                </button>
                <div className="w-full">
                    <div className="mt-[20px] flex flex-row gap-[20px] border-b-[1px] border-[#DDDDDD] border-solid">
                        <button onClick={handleLanguageAndLocal} className={`p-[16px] ml-[-16px] ${showLanguageAndLocal ? 'decoration-slice underline underline-offset-[20px] text-black' : 'text-[#717171]'}`}>
                            언어와 지역
                        </button>
                        <button onClick={handleExchange} className={`p-[16px] ml-[-16px] ${showExchange ? 'decoration-slice underline underline-offset-[20px] text-black' : 'text-[#717171]'}`}>
                            통화
                        </button>
                    </div>
                        ...
                        <div className="">
                            <section className="relative flex flex-col justify-center text-[16px] bg-[#F7F7F7] rounded-[12px] w-[326px] h-[80px] mt-[50px] p-[16px]">
                                ...
                                <div className="absolute left-[260px]">
                                    {autoTranslation && 
                                        <button type="button" onClick={handleAutoTranslation} className="flex flex-row justify-end items-center w-[50px] h-[32px] py-[1px] border-[1px] border-black bg-black rounded-[20px]">
                                    ...
                                    {!autoTranslation && 
                                        <button type="button" onClick={handleAutoTranslation} className="flex flex-row items-center w-[50px] h-[32px] py-[1px] border-[1px] border-[#B0B0B0] bg-[#B0B0B0] rounded-[20px]">
                                            <div className="border-[1px] border-solid border-[#B0B0B0] rounded-[50%] h-[30px] w-[30px] bg-[#fff]">
                                            </div>
                                        </button>
                                    }
                                </div>
                            ...
                            <section>
                                ...
                                <ul className="grid gap-[10px] bnb_xl:grid-cols-5 bnb_lg:grid-cols-4 bnb_md:grid-cols-3">
                                    {globalDummy.map((el) => 
                                        <li key={el.id} className="flex flex-col flex-grow justify-center hover:bg-[#F7F7F7] rounded-[12px] cursor-pointer h-[56px] px-[13px] py-[10px]">
            ...
            {/* 모달 밖 (어두운 배경) */}
            <div onClick={closeGlobalModal} className="fixed top-0 left-0 w-full h-full z-[101] bg-[var(--modal-outside-bg)]">
            </div>
        </div>
    )
}