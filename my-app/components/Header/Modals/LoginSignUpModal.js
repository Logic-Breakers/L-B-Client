    ...
    <div>
      ...
      <section className="border-[1px] border-solid border-black w-[568px] max-h-[746px] z-[1000] fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] bg-[#fff] rounded-[12px] cursor-default">
        <header className="flex flex-row items-center gap-[165px] h-[63px] px-[24px] border-b-[1px] border-solid border-[#DDDDDD]">
          <button type="button" onClick={closeLoginModal} className={`text-[16px] px-[8px] py-[2px] rounded-[20px] hover:shadow-inner hover:bg-[#F7F7F7]`}>
            ✕
          </button>
        ...
        <section className="mt-[30px] px-[24px]">
          ...
          <form onSubmit={handleSubmit}>
            <label htmlFor="country" className="relative flex flex-col justify-center border-[1px] border-solid border-[#cccccc] rounded-t-[10px] w-full h-[55px] pl-[12px] pr-[20px] ">
              <div className="text-[12px] text-gray-500">
                국가/지역
              </div>
              <select id="country" className="appearance-none" defaultValue={'82'}>
              ...
              <FontAwesomeIcon icon={faChevronDown} className="absolute left-[480px]" />
            </label>
            
            <input required type="tel" onChange={getPhoneNumber} placeholder="전화번호" className="border-[1px] border-solid border-[#cccccc] mt-[-1px] w-full rounded-b-[12px] h-[55px] px-[10px] appearance-none">
            </input>
            ...
            <button type='submit' onClick={handleSubmit} className="w-full h-[45px] rounded-[12px] text-white bg-[#E31C5B] active:scale-95 ease-in duration-[0.1s] " >
              계속
            </button>
          </form>  
        </section>
        ...
        <section className="my-[20px] px-[24px]">
          ...
          {/* 구글 소셜로그인 */}
          <button type="button" className="w-full h-[48px] border-[1px] border-solid border-black rounded-[10px] hover:bg-[#F7F7F7] flex flex-row items-center px-[23px] gap-[170px]">
